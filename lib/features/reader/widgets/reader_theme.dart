...
class ReaderThemeScope extends ConsumerWidget {
  const ReaderThemeScope({super.key, required this.child});

  final Widget child;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final brightness = ref.watch(appSettingsProvider.select(readerBrightness));
...
Brightness? readerBrightness(AppSettings settings) =>
...
/// 亮暗由背景色定死，不给切。
bool readerThemeLocked(AppSettings settings) =>
    settings.readerBackgroundMode == ReaderBackgroundMode.custom;

/// 夜间开关只改阅读页的亮暗，不碰全局 `theme`。目标亮暗与全局一致时写回 followApp，
/// 免得把「跟随系统」换成固定值后再也回不去。
void toggleReaderNightMode(BuildContext context, WidgetRef ref) {
  ...
  ref
      ...
      .update((settings) => settings.copyWith(readerTheme: readerTheme));
}
